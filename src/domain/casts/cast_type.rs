...
impl FromStr for CastType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(Error::mapping("Cast", &format!("Unknown cast type: {}", s))),
        }
    }
}

impl CastType {
    pub fn cast_from_value(&self, value: &Value) -> Result<Value, Error> {
        match (self, value) {
            ...
            (CastType::Bool, Value::String(s)) => match s.to_lowercase().as_str() {
                ...
                _ => Err(Error::mapping("Cast", "Cannot cast string to bool")),
            },
            ...
            (CastType::Int, Value::String(s)) => s
                .parse::<i64>()
                .map(Value::Int)
                .map_err(|_| Error::mapping("Cast", "Cannot cast string to int")),
            ...
            (CastType::Float, Value::String(s)) => s
                .parse::<f64>()
                .map(Value::Float)
                .map_err(|_| Error::mapping("Cast", "Cannot cast string to float")),
    ...
    pub fn cast_to_value(&self, value: &Value) -> Result<Value, Error> {
        Ok(value.clone())
    }
}
