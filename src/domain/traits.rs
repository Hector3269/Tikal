use crate::infrastructure::core::types::{DbRow, Value};
...
pub trait FromRow {
    fn from_row(row: HashMap<String, Value>) -> Result<Self, TikalError>
    where
        Self: Sized;
}
...
pub trait ActiveRecord: Entity + Identifiable + Sized {
    ...
    fn from_row(row: DbRow) -> Result<Self, TikalError>;
    ...
    /// Delete the entity
    async fn delete<E: QueryExecutor + Send + Sync>(&self, executor: &E) -> Result<(), TikalError>;
}
    ...
    ActiveRecord + From<DbRow> + Timestamps + SoftDeletes + Send + Sync + 'static
{
    ...
    /// Get all records
    async fn all<E: QueryExecutor + Send + Sync>(executor: &E) -> Result<Vec<Self>, TikalError> {
